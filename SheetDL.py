...
def check_dependencies():
    ...
    if missing:
        ...
        if DarkMessageBox.askyesno("Missing Dependencies", msg):
            root.destroy()
            ...
            DarkMessageBox.showerror("Cannot Continue", "Required dependencies are missing. The application will now exit.")
            root.destroy()
            sys.exit(1)
...
def check_for_updates(silent=False):
    ...
    try:
        ...
        if response.status_code != 200:
            if not silent:
                DarkMessageBox.showinfo("Update Check", "Could not check for updates. GitHub may be unavailable.")
            return False
        ...
        if not remote_version:
            if not silent:
                DarkMessageBox.showinfo("Update Check", "Could not determine remote version.")
            return False
        ...
        if remote_ver > local_ver:
            ...
            if silent:
                # In silent mode, don't prompt - just return True to indicate update available
                return True

            msg = f"A new version of SheetDL is available!\n\nCurrent version: {VERSION}\nNew version: {remote_version}\n\nWould you like to update now?"
            if DarkMessageBox.askyesno("Update Available", msg):
                ...
                try:
                    ...
                    DarkMessageBox.showerror("Update Failed", f"Could not create backup: {e}")
                    return False
                ...
                try:
                    ...
                    DarkMessageBox.showinfo("Update Complete", f"SheetDL has been updated to version {remote_version}!\n\nThe application will now restart.")
                    ...
                    try:
                        ...
                        pass
                    DarkMessageBox.showerror("Update Failed", f"Could not write update: {e}\n\nThe original file has been restored.")
                    return False
            return True
        else:
            if not silent:
                DarkMessageBox.showinfo("No Updates", f"You are running the latest version ({VERSION}).")
            return False
            
    except requests.exceptions.Timeout:
        if not silent:
            DarkMessageBox.showinfo("Update Check", "Update check timed out. Please check your internet connection.")
        return False
    except Exception as e:
        if not silent:
            DarkMessageBox.showinfo("Update Check", f"Could not check for updates: {e}")
        return False
...
class RoundedCard(tk.Frame):
    def __init__(self, parent, title, colors, radius=12, padding=16):
    ...
    def _redraw(self, event=None):
        ...
        # Draw inner panel
        create_round_rect(
            self.canvas, 1, 1, width-1, height-1,
            radius=self.radius,
...
class RoundedButton(tk.Frame):
    def __init__(self, parent, text, command, colors, width=160, height=38):
...
class DarkMessageBox:
    """Custom dark-themed message boxes"""
    @staticmethod
    def showinfo(title, message, parent=None):
        """Show an info message box with dark theme"""
        ...
        # Message frame
        msg_frame = tk.Frame(dialog, bg="#1a1a1a", padx=20, pady=20)
        ...
        # OK button
        button_frame = tk.Frame(msg_frame, bg="#1a1a1a")
        button_frame.pack()
        ...
        ok_btn.pack()
        ok_btn.bind("<Enter>", lambda e: ok_btn.config(bg="#34d399"))
        ok_btn.bind("<Leave>", lambda e: ok_btn.config(bg="#10b981"))

        # Update and center the dialog
    ...
    def showwarning(title, message, parent=None):
        """Show a warning message box with dark theme"""
        DarkMessageBox.showinfo(title, message, parent)
    ...
    def askyesno(title, message, parent=None):
        """Show a yes/no question dialog with dark theme"""
        ...
        yes_btn.bind("<Enter>", lambda e: yes_btn.config(bg="#34d399"))
        yes_btn.bind("<Leave>", lambda e: yes_btn.config(bg="#10b981"))
        ...
        no_btn.bind("<Enter>", lambda e: no_btn.config(bg="#3a3a3a"))
        no_btn.bind("<Leave>", lambda e: no_btn.config(bg="#2a2a2a"))
...
class MusicDownloaderGUI:
    def __init__(self, root):
        ...
        self.colors = {
            ...
            "text_dim": "#888888",
            "border": "#2a2a2a",
            "secondary": "#262626"  # Added default value for 'secondary'
        }
        ...
        # Check for updates after window is initialized (silent mode)
        self.root.after(500, lambda: check_for_updates(silent=True))
    ...
    def setup_title_bar(self):
        self.title_bar = tk.Frame(self.root, bg="#141414", relief='flat', bd=0, height=44)
        ...
        self.title_label.pack(side=tk.LEFT, padx=(18, 8))
        ...
        self.author_canvas.pack(side=tk.LEFT, pady=10)
        ...
        self._author_bg = create_round_rect(
            ...
            radius=6, fill="#0a0a0a", outline="#1f1f1f"
        )
        ...
        control_frame = tk.Frame(self.title_bar, bg="#141414")
        control_frame.pack(side=tk.RIGHT, padx=8)

        self.close_btn = self._create_title_button(control_frame, "✕", self.close_window, hover_bg="#dc2626")
        ...
        self.title_shadow = tk.Frame(self.root, bg="#0a0a0a", height=1)
    ...
    def _create_title_button(self, parent, symbol, command, hover_bg=None):
        hover_color = hover_bg or "#2a2a2a"
        btn = tk.Label(
            parent,
            text=symbol,
            bg="#141414",
        ...
        btn.pack(side=tk.RIGHT, padx=1, pady=8)
        btn.bind("<Button-1>", lambda e: command())
        btn.bind("<Enter>", lambda e: (btn.config(bg=hover_color, fg="#ffffff")))
        btn.bind("<Leave>", lambda e: btn.config(bg="#141414", fg="#888888"))
        return btn

    def _set_initial_icon(self):
        """Set icon before overrideredirect for proper taskbar icon"""
        icon_path = Path(__file__).parent / 'DL.ico'
        if icon_path.exists():
            try:
    ...
    def toggle_max_restore(self):
        if not self.is_maximized:
            ...
            # Set maximized state BEFORE changing geometry
            ...
            # Update window to ensure geometry change takes effect
            self.root.update_idletasks()

            # Apply window rounding (will use radius=0 for maximized)
            self.apply_window_rounding()
        else:
            # Set restored state BEFORE changing geometry
            self.is_maximized = False
            self.max_btn.config(text="□")
            ...
            self._restore_resize_handles()

            # Apply window rounding (will use normal radius for non-maximized)
            self.apply_window_rounding()
    ...
    def apply_window_rounding(self):
        ...
        try:
            ...
            # Add 1 to width and height to prevent clipping
    ...
    def create_resize_handles(self):
        ...
        self.resize_handles = {}
        for direction, spec in self.resize_specs.items():
    ...
    def _restore_resize_handles(self):
        """Restore resize handles after un-maximizing"""
        for direction, handle in self.resize_handles.items():
            spec = self.resize_specs[direction]
            handle.place(**{k: v for k, v in spec.items() if k not in ('cursor',)})
    ...
    def setup_ui(self):
        ...
        style.configure('Card.TLabelframe.Label', background=self.colors["panel"], foreground=self.colors["accent"], font=('Segoe UI Semibold', 10))
        style.configure('TLabel', background=self.colors["panel"], foreground=self.colors["text"], font=('Segoe UI', 10))
        style.configure('Primary.TButton', background=self.colors["accent"], foreground='#ffffff', padding=(20, 8), relief='flat')
        style.map('Primary.TButton', background=[('active', self.colors["highlight"])])
        style.configure('Link.TButton', background=self.colors["panel"], foreground=self.colors["accent"], padding=(10, 4), relief='flat')
        style.map('Link.TButton', foreground=[('active', self.colors["text"])] )
        
        # Entry styling - sleek dark inputs
        ...
        style.map('TCombobox',
            fieldbackground=[('readonly', '#0d0d0d'), ('disabled', '#0a0a0a'), ('focus', '#121212')],
            foreground=[('readonly', self.colors["text"]), ('disabled', self.colors["text_dim"])],
            background=[('readonly', self.colors["panel"])],
            selectbackground=[('readonly', '#0d0d0d'), ('focus', '#121212')],
        ...
        # Scrollbar styling - minimal dark scrollbar
        ...
        style.map('Vertical.TScrollbar',
            background=[('active', '#333333'), ('pressed', self.colors["accent"])]
        )
        # Checkbox: sleek with accent color
        ...
        style.map('TCheckbutton',
            ...
            indicatorcolor=[('selected', self.colors["accent"]), ('pressed', self.colors["highlight"])]
        )
        style.configure('Accent.Horizontal.TProgressbar', background=self.colors["accent"], troughcolor='#1a1a1a', thickness=6)
        ...
        self.content_frame = tk.Frame(self.canvas, bg=self.colors["background"], padx=24, pady=20)
        ...
        sheets_section = RoundedCard(main_frame, "Google Sheets", self.colors)
        ...
        self.test_connection_btn = RoundedButton(sheet_btn_frame, "Test Connection", self.test_connection, self.colors, width=130, height=34)
        self.test_connection_btn.pack(side=tk.TOP, pady=(0, 4))
        
        self.queue_btn = RoundedButton(sheet_btn_frame, "Add to Queue", self.add_to_queue, self.colors, width=130, height=34)
        ...
        self.browse_btn = RoundedButton(output_frame, "Browse", self.browse_folder, self.colors, width=100, height=34)
        ...
        ttk.Label(output_frame, text="YouTube Format:").grid(row=4, column=0, sticky=tk.W, pady=(15, 0))
        ...
        yt_format_combo.grid(row=4, column=1, sticky=tk.W, padx=5, pady=(15, 0))
        ...
        self.log_text = scrolledtext.ScrolledText(
            ...
            takefocus=0,  # Reduce focus changes
        ...
        self.log_text.configure(
            ...
            padx=12,
            pady=8,
            blockcursor=False,  # Faster cursor rendering
            inactiveselectbackground=self.colors["secondary"]  # Better selection visual
        )
        for event_name in (
        ...
        self.download_btn = RoundedButton(button_frame, "Start Download", self.start_download, self.colors, width=160, height=36)
        self.download_btn.grid(row=0, column=0, padx=6)
        
        self.pause_btn = RoundedButton(button_frame, "Pause", self.toggle_pause, self.colors, width=90, height=36)
        self.pause_btn.grid(row=0, column=1, padx=6)
        self.pause_btn.set_state('disabled')
        
        self.stop_btn = RoundedButton(button_frame, "Stop", self.stop_download, self.colors, width=90, height=36)
        self.stop_btn.grid(row=0, column=2, padx=6)
        self.stop_btn.set_state('disabled')
        
        self.save_btn = RoundedButton(button_frame, "Save Settings", self.save_settings, self.colors, width=130, height=36)
        self.save_btn.grid(row=0, column=3, padx=6)
        
        self.update_btn = RoundedButton(button_frame, "Check Updates", self.check_for_updates_gui, self.colors, width=130, height=36)
        ...
        self.queue_frame.grid(row=5, column=0, pady=(12, 0), sticky='ew')
        self.queue_frame.grid_remove()  # Hidden by default
        
        queue_header = tk.Frame(self.queue_frame, bg="#151515", height=32)
        ...
        self.queue_label.pack(side=tk.LEFT, padx=12, pady=6)
        ...
        self.clear_queue_btn.pack(side=tk.RIGHT, padx=12)
        self.clear_queue_btn.bind("<Button-1>", lambda e: self.clear_queue())
        self.clear_queue_btn.bind("<Enter>", lambda e: self.clear_queue_btn.config(fg=self.colors["highlight"]))
        self.clear_queue_btn.bind("<Leave>", lambda e: self.clear_queue_btn.config(fg=self.colors["accent"]))
        ...
        self.queue_listbox.pack(fill=tk.X, padx=1, pady=(1, 8))
        self.queue_listbox.bind("<Double-Button-1>", self.remove_queue_item)

        self.report_environment()
        
    def log(self, message):
        """Add message to log with optimized updates"""
        ...
        # Only update UI periodically to reduce lag
        if not hasattr(self, '_log_update_count'):
            self._log_update_count = 0
        self._log_update_count += 1

        # Update every 5 logs or if autoscroll is on (user is watching)
        if self._log_update_count % 5 == 0 or self.log_auto_follow:
            self.root.update_idletasks()

        # Only refresh state every 3 logs to reduce overhead
        if self._log_update_count % 3 == 0:
            self.refresh_log_follow_state()
    ...
    def save_settings(self):
        ...
        DarkMessageBox.showinfo("Success", "Settings saved!")
    ...
    def add_to_queue(self):
        ...
        if not url:
            DarkMessageBox.showerror("Error", "Please enter a Google Sheets URL to add to queue")
            return
        ...
        for item in self.download_queue:
            if item['url'] == url and item['gid'] == gid:
                DarkMessageBox.showinfo("Already in Queue", "This sheet is already in the queue.")
                return
    ...
    def clear_queue(self):
        ...
        if self.download_queue:
            if DarkMessageBox.askyesno("Clear Queue", f"Remove all {len(self.download_queue)} items from the queue?"):
    ...
    def test_connection(self):
        ...
        try:
            ...
            if not sheet_url:
                DarkMessageBox.showerror("Error", "Please enter a Google Sheets URL")
                return
                
            # Try to access the sheet (public read-only)
            sheet_id = self.extract_sheet_id(sheet_url)
            if not sheet_id:
                DarkMessageBox.showerror("Error", "Invalid Google Sheets URL")
                return
            ...
            # Get sheet title
            sheet_title = self.get_sheet_title(sheet_url, sheet_id)
                ...
                try:
                    ...
                    if response.status_code == 200 and len(response.text) > 50:
                        ...
                        if len(lines) > 0:
                            ...
                            self.log(f"✓ Connection successful! Found {row_count} rows")
                            self.log(f"Headers: {lines[0][:100]}...")
            ...
            # Show success dialog AFTER all processing is done
            if success:
                # Build message
                if sheet_title:
                ...
                # Ensure UI is fully updated before showing dialog
                self.root.update_idletasks()
                DarkMessageBox.showinfo("Success", message)
            
            if not success:
                self.log("✗ All connection methods failed")
                DarkMessageBox.showwarning("Connection Issue", 
            ...
            DarkMessageBox.showerror("Error", f"Connection failed: {str(e)}")
    ...
    def extract_embedded_hyperlinks(self, sheet_id, gid):
        """Extract hyperlinks embedded in cells from Google Sheets.
        
        This is needed for sheets where URLs are in HYPERLINK() formulas
        but the display text is just the format (e.g., 'MP3', 'WAV', 'Download').
        The CSV export only returns the display text, not the underlying URL.
        
        Uses ODS export which preserves hyperlinks, falling back to view page scraping.
        
        Returns a list of URLs found in the sheet.
        """
        all_urls = []
        
        # List of download hosts to look for
        download_hosts = [
            'pillowcase', 'pillows.su', 'plwcse', 'pixeldrain', 'krakenfiles',
            'youtube.com/watch', 'youtu.be', 'soundcloud.com', 'on.soundcloud.com',
            'mega.nz', 'mega.co.nz', 'drive.google.com', 'mediafire.com',
            'gofile.io', 'fileditch', 'froste.lol', 'bumpworthy.com',
            'imgur.com', 'imgur.gg', 'catbox.moe', 'archive.org', 'streamable.com',
            's3.amazonaws.com', 'dump.li'
        ]
        ...
        # Method 1: Try ODS export (contains ALL hyperlinks)
        try:
            self.log("  Trying ODS export for hyperlinks...")
            ods_url = f'https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=ods&gid={gid}'
            response = requests.get(ods_url, headers=headers, timeout=120)
            ...
            if response.status_code == 200 and len(response.content) > 1000:
                ...
                with zipfile.ZipFile(BytesIO(response.content)) as zf:
                    for name in zf.namelist():
                        try:
                            ...
                            # Find hyperlinks in ODS format (xlink:href)
                            href_urls = re.findall(r'xlink:href="([^"]+)"', content)
                            ...
                            for u in href_urls + plain_urls:
                                # Clean URL
                                u = u.replace('&amp;', '&').rstrip('\\').rstrip(',').rstrip('"')
                                # Check if it's a download host
                                if any(h in u.lower() for h in download_hosts):
                ...
                if all_urls:
                    self.log(f"  ✓ ODS export: found {len(all_urls)} hyperlinks")
                    return all_urls
                    
        except Exception as e:
            self.log(f"  ODS export failed: {str(e)[:50]}")
        
        # Method 2: Fallback to view page scraping (limited to visible rows)
        try:
            self.log("  Falling back to view page scraping...")
            ...
            url_patterns = [
                ...
                r'https?://(?:www\.)?p{1,2}illows\.su/[^\s"\'\\<>]+',
                r'https?://(?:www\.)?p{1,2}illowcase\.su/[^\s"\'\\<>]+',
                r'https?://plwcse\.top/[^\s"\'\\<>]+',
                r'https?://(?:www\.)?pillowcase\.zip/[^\s"\'\\<>]+',
                ...
                r'https?://(?:www\.)?streamable\.com/[a-zA-Z0-9]+',
            ...
            # Also try the escaped JSON format: \"24\":\"URL\"
            escaped_urls = re.findall(r'\\"24\\":\s*\\"(https?://[^\\]+)\\"', response.text)
            for url in escaped_urls:
                if url not in all_urls and any(h in url.lower() for h in download_hosts):
                    all_urls.append(url)
            
            if all_urls:
                self.log(f"  ✓ View page: found {len(all_urls)} hyperlinks (may be limited)")
    ...
    def start_download(self):
        ...
        if not self.output_folder_var.get():
            DarkMessageBox.showerror("Error", "Please select an output folder")
            return
    ...
    def download_process(self):
        ...
        try:
            ...
            if not response or response.status_code != 200 or len(response.text) == 0:
                ...
                DarkMessageBox.showerror("Error", "Cannot access sheet. Please use 'Test Connection' first to verify access.")
                return
            ...
            if not all_rows:
                ...
                DarkMessageBox.showerror("Error", "Sheet appears to be empty.")
                return
            ...
            def extract_header_name(cell_text):
                ...
                header_patterns = [
                    ...
                    (r'^(Quality)\b', 'Quality'),
                    (r'^(Main Link)\b', 'Main Link'),
                    (r'^(Alternate Link)', 'Alternate Links'),
                    (r'^(Link\(?s?\)?)\b', 'Link(s)'),
                    (r'^(Artist)\b', 'Artist'),
                    (r'^(Title)\b', 'Title'),
                    (r'^(Album)\b', 'Album'),
            ...
            if total_rows == 0:
                DarkMessageBox.showwarning("No Data", "No rows with data were found after the header.")
                return
            ...
            # Support various link column names - Main Link is primary, Alternate Links is fallback
            url_col = find_best_column(['Main Link', 'Link(s)', 'Links', 'Link', 'URL', 'Download', 'Source', 'File'])
            alt_url_col = find_best_column(['Alternate Links', 'Alt Link', 'Backup', 'Mirror'])
            ...
            # Check if URL column has actual URLs or just format text like "MP3", "WAV", "Download"
            embedded_hyperlinks = []
            ...
            if not url_col or not urls_found_in_csv:
                ...
                self.log("URL column contains format text (like 'MP3' or 'Download'), not actual URLs.")
            ...
            embedded_exhausted_logged = False  # Track if we've logged the exhausted message
            
            # If we have embedded hyperlinks, log it but continue to normal processing
            if embedded_hyperlinks:
                self.log(f"\n=== Using Embedded Hyperlinks Mode ===")
                self.log(f"Found {len(embedded_hyperlinks)} embedded download links")
                self.log(f"Will match with {total_rows} rows from spreadsheet...")
                self.log(f"NOTE: Google Sheets only exposes URLs from visible rows.")
                self.log(f"      Some downloads may be skipped if the sheet has many rows.")
            
            # Process rows with URL column (normal mode)
            # embedded_hyperlinks will be used as fallback when cell has "Download" text instead of URL
            ...
            for idx, row in enumerate(rows):
                ...
                try:
                    ...
                    alt_url_cell = (row.get(alt_url_col, "") or "").strip() if alt_url_col else ""
                    ...
                    # Extract URLs from the main link cell
                    urls_in_cell = self.extract_urls_from_cell(url_cell)
                    
                    # Also extract from alternate links column and append as fallbacks
                    if alt_url_cell:
                        alt_urls = self.extract_urls_from_cell(alt_url_cell)
                        for alt_url in alt_urls:
                            if alt_url not in urls_in_cell:
                                urls_in_cell.append(alt_url)
                    
                    # If no URLs in cell but we have embedded hyperlinks, try to use one
                    if not urls_in_cell and embedded_hyperlinks:
                        # Check if url_cell contains format indicators or link text
                        format_indicators = ['mp3', 'wav', 'flac', 'm4a', 'aac', 'ogg', 'wma', 'aiff', 'alac']
                        link_indicators = ['download', 'link', 'snippet', 'youtube', 'soundcloud', 'archive', 'streamable', 'tweet']
                        cell_lower = url_cell.lower().strip()
                        is_format = cell_lower in format_indicators or any(cell_lower.startswith(f) for f in format_indicators)
                        is_link_text = any(ind in cell_lower for ind in link_indicators)
                        if is_format or is_link_text:
                            ...
                            elif not embedded_exhausted_logged:
                                self.log(f"\n⚠ Embedded URLs exhausted ({len(embedded_hyperlinks)} found)")
                                self.log(f"  Remaining rows with '{url_cell}' text will be skipped.")
                                self.log(f"  This is a Google Sheets limitation - not all hyperlinks are accessible.")
                                embedded_exhausted_logged = True
                    ...
                    # Try URLs in order - stop after first successful download
                    # This prevents downloading multiple copies of the same song
                    for link_idx, url in enumerate(urls_in_cell, start=1):
                        display_url = url if len(url) <= 100 else f"{url[:100]}..."
                        prefix = f"  URL {link_idx}/{len(urls_in_cell)}: " if len(urls_in_cell) > 1 else "  URL: "
                        ...
                        if 'pillows.su' in lowered or 'plwcse.top' in lowered or 'pillowcase.zip' in lowered or 'pillowcase.su' in lowered:
                            ...
                            self.log("    Type: Archive.org")
                        elif 'streamable.com' in lowered:
                            self.log("    Type: Streamable")
                        ...
                        # Use original title (don't append "Link 1", "Link 2" since we only download one)
                        link_title = title
                        ...
                        if success:
                            ...
                            self.log("    ✓ SUCCESS")
                            # Stop trying other URLs for this song - we got one!
                            if len(urls_in_cell) > 1:
                                self.log(f"    → Skipping {len(urls_in_cell) - link_idx} alternate link(s)")
                            break
                        else:
                            # Only log as failed if this was the last URL to try
                            if link_idx == len(urls_in_cell):
                                fail_count += 1
                                ...
                                self.log("    ✗ FAILED (all links exhausted)")
                            else:
                                self.log("    ✗ Failed, trying next link...")
            ...
            DarkMessageBox.showinfo("Complete", f"Download finished!\nSuccess: {success_count}\nFailed: {fail_count}")
            
        except Exception as e:
            self.log(f"✗ Fatal error: {str(e)}")
            DarkMessageBox.showerror("Error", f"Download failed: {str(e)}")
    ...
    def download_file(self, url, output_path, artist, title):
        ...
        try:
            ...
            if 'pillows.su' in url:
                ...
                return self.download_archive_org(url, output_path, artist, title)
            elif 'streamable.com' in url:
                return self.download_streamable(url, output_path, artist, title)
            elif 'bandcamp.com' in url:
                return self.download_bandcamp(url, output_path, artist, title)
            elif any(x in url for x in ['youtube.com', 'youtu.be', 'soundcloud.com', 'on.soundcloud.com', 'vimeo.com']):
    ...
    def download_youtube(self, url, output_path, artist, title):
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'no_warnings': True,
            'writethumbnail': True,  # Download thumbnail/cover art
        ...
        # Add thumbnail embedding postprocessor if FFmpeg is available
        if ffmpeg_path and ydl_opts.get('embedthumbnail'):
            postprocessors.append({
                'key': 'EmbedThumbnail',
                'already_have_thumbnail': False,
            })

        # Add metadata postprocessor if FFmpeg is available
        if ffmpeg_path:
            postprocessors.append({
                'key': 'FFmpegMetadata',
                'add_metadata': True,
        ...
        # Set postprocessors if any were added
        if postprocessors:
    ...
    def download_pillows(self, url, output_path, artist, title):
        """Download from pillows.su using multiple API detection methods"""
        try:
            ...
            if not file_id_match:
                self.log(f"  ✗ Could not extract file ID from URL")
                return False
            
            file_id = file_id_match.group(1)
            self.log(f"  → File ID: {file_id}")
            
            # Get the page to detect file info
            response = requests.get(url, headers=headers, timeout=15, allow_redirects=True)
            response.raise_for_status()
            
            # Parse HTML to find download link and file extension
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # Try multiple methods to construct the correct download URL
            download_link = None
            file_ext = None
            
            # Method 1: Look for download button or audio-specific links
            self.log(f"  → Method 1: Searching for download link...")
            
            # Look for links with "download" text or class names
            for link in soup.find_all('a', href=True):
                link_text = link.get_text().lower().strip()
                link_class = ' '.join(link.get('class', [])).lower()
                href = link.get('href', '')
                
                # Skip cover/image links explicitly
                if '/cover/' in href or '.webp' in href or '.jpg' in href or '.png' in href:
                    continue
                    
                # Look for download-specific links with API URLs
                if 'api.pillows.su' in href and file_id in href:
                    # Prioritize links with download indicators
                    if 'download' in link_text or 'download' in link_class or 'audio' in href or 'file' in href:
                        download_link = href
                        self.log(f"  ✓ Found download link: {download_link}")
                        break
            
            # Method 2: Check for file extension in page metadata or links
            if not file_ext:
                # Look for extension hints in the page
                for ext in ['.mp3', '.m4a', '.wav', '.flac', '.ogg', '.aac', '.opus']:
                    if ext in response.text.lower():
                        file_ext = ext
                        self.log(f"  → Detected extension: {file_ext}")
                        break
            
            # Method 3: Use the correct pillows.su API endpoint directly
            # The download API is: https://api.pillows.su/api/download/{file_id}
            if not download_link:
                download_link = f"https://api.pillows.su/api/download/{file_id}"
                self.log(f"  → Using API download endpoint")
            ...
            self.log(f"  → Downloading from API...")
            try:
                response = requests.get(download_link, headers=download_headers, stream=True, timeout=30)
                response.raise_for_status()
            except (requests.exceptions.Timeout, requests.exceptions.ReadTimeout):
                self.log(f"  ✗ API timed out (server too slow)")
                self.log(f"  ⚠ Skipping - try again later when server is responsive")
                return False
            except requests.exceptions.HTTPError as e:
                self.log(f"  ✗ HTTP Error {e.response.status_code}: API endpoint may be incorrect")
                return False
            except Exception as e:
                self.log(f"  ✗ Connection failed: {str(e)[:100]}")
                return False
            ...
            # Build friendly filename with detected or inferred extension
            if not file_ext:
                # Try to get from URL path
                file_ext = Path(urlparse(download_link).path).suffix
            if not file_ext:
                # Infer from content-type
                file_ext = self.infer_extension(response.headers.get('Content-Type'))
            if not file_ext:
                file_ext = '.mp3'  # Default fallback
            
            filename = self.build_track_filename(title, extension=file_ext)
    ...
    def download_krakenfiles(self, url, output_path, artist, title):
        """
        Download from krakenfiles.com with intelligent Cloudflare bypass

        Three-tier download strategy:
        1. Try cloudscraper (fast, basic CF bypass) for original file
        2. If blocked by CAPTCHA, try undetected-chromedriver (slower but bypasses CAPTCHA)
        3. Fall back to m4a audio stream (always works, no CAPTCHA)

        To enable Selenium bypass, install: pip install undetected-chromedriver
        ...
        try:
            ...
            # Method 1: Try cloudscraper first (fast)
            try:
                import cloudscraper
                ...
                self.log(f"  → cloudscraper not available")
            ...
            if original_extension:
                try:
                    ...
                    if dl_response.status_code == 200 and 'text/html' not in content_type.lower():
                        ...
                        cf_blocked = True
                        self.log(f"  → Cloudflare CAPTCHA detected - trying advanced bypass...")

                except Exception as e:
                    self.log(f"  → Original download failed ({str(e)})")
                    cf_blocked = True
    ...
    def download_archive_org(self, url, output_path, artist, title):
        """Download from archive.org (Internet Archive)"""
        try:
            self.log(f"  → Downloading from Archive.org...")
            
            # Check if it's a direct file link or a details page
            if '/download/' in url or url.endswith(('.mp3', '.wav', '.flac', '.m4a', '.ogg', '.mp4')):
                # Direct file download
                return self.download_direct(url, output_path, artist, title)
            
            # For archive.org pages, try yt-dlp first (handles audio/video well)
            base_filename = self.build_safe_title(title)
            ...
            try:
                ...
                self.log(f"  → yt-dlp failed, trying direct download...")
                # Fallback to direct download
                return self.download_direct(url, output_path, artist, title)
                
        except Exception as e:
            self.log(f"  ✗ Archive.org error: {str(e)}")
            return False

    def download_streamable(self, url, output_path, artist, title):
        """Download from Streamable"""
        try:
            self.log(f"  → Downloading from Streamable...")
            
            base_filename = self.build_safe_title(title)
            
            # Streamable works well with yt-dlp
            ydl_opts = {
                'outtmpl': str(output_path / f"{base_filename}.%(ext)s"),
                'noplaylist': True,
                'quiet': True,
                'no_warnings': True,
                'format': 'best',  # Streamable has video, get best quality
            }
            ...
            try:
                ...
                self.log(f"  ✗ Streamable error: {str(e)}")
                return False
    ...
    def download_bandcamp(self, url, output_path, artist, title):
        """Download from Bandcamp using yt-dlp"""
        try:
            base_filename = self.build_safe_title(title)

            # Bandcamp is music-focused, so default to audio format
            # Check if user has set a format preference, otherwise use audio_m4a
            yt_format = self.yt_format_var.get() if hasattr(self, 'yt_format_var') else 'audio_m4a'

            # For Bandcamp, prefer audio formats even if video is selected
            if yt_format.startswith('video_'):
                yt_format = 'audio_m4a'

            ydl_opts = {
                'outtmpl': str(output_path / f"{base_filename}.%(ext)s"),
                'noplaylist': False,  # Bandcamp albums should download as playlists
                'quiet': True,
                'no_warnings': True,
                'writethumbnail': True,  # Download album/cover art
                'embedthumbnail': True,  # Embed thumbnail in audio file
                'addmetadata': True,  # Add metadata to file
            }
            ...
            self.log(f"    Downloading from Bandcamp...")

            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ydl.download([url])
                self.log(f"  ✓ Bandcamp download complete")
                return True
            ...
            self.log(f"  ✗ Bandcamp error: {str(e)}")
            return False
